...
class AsyncStorageAdapter implements Adapter {
	...
	async save<T extends PersistentModel>(
		...
		console.log('Namespace+Model', namespaceName, model);
		...
		console.log('Array', [storeName, ...Array.from(set.values())]);
		...
		for await (const resItem of connectionStoreNames) {
			console.log('RESULT_ITEM', resItem);
			...
			console.log('SAVED_ITEM', savedItem);
		}
	...
	private async load<T>(
		...
		console.log('LOAD_RELATION', relations);
			...
			if (predicates) {
				...
				if (idPredicate) {
					...
					if (record) {
						...
						console.log('QUERY_BY_ID RECORD', [x]);
						return [x];
					}
					return [];
				}
				console.log('Query All', storeName);
	...
	async delete<T extends PersistentModel>(
		...
		if (isModelConstructor(modelOrModelConstructor)) {
			...
			const models = await this.query(modelConstructor, condition); //check if this is paginated.
			// RNTODO: refactor this to use a function like getRelations()
			const relations = this.schema.namespaces[nameSpace].relationships[
				modelConstructor.name
			].relationTypes;
			console.log('STORE, RELATIONS', storeName, relations);
			/**
			...
			if (condition !== undefined) {
				...
				// Models is parents, deleted models is all relations
			...
			if (condition) {
				...
				const isValid = validatePredicate(fromDB, type, predicateObjs); // Check condition against instance.
				console.warn('BEFORE IS VALID');
				if (!isValid) {
					const msg = 'Conditional update failed';
					logger.error(msg, { model: fromDB, condition: predicateObjs });
		...
		for await (const rel of relations) {
			...
			switch (relationType) {
				case 'HAS_ONE':
					...
					break;
				case 'HAS_MANY':
					console.log('DELETE_INDEX, storeName', index, storeName);
					for await (const model of models) {
						const allRecords = await AsyncStorageDatabase.getAll(storeName);
